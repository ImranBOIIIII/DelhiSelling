  ...
  return (
    <div className="p-8">
            ...
            <div key={order.id} className="border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-3 mt-6 pt-6 border-t border-gray-200">
                  ...
                  <button onClick={() => openTrack(order)} className="flex items-center space-x-2 text-blue-600 hover:text-blue-700 px-4 py-2 rounded-lg border border-blue-200 hover:bg-blue-50 transition-colors">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                  ...
                  <div className="text-sm font-mono font-bold text-blue-900">{selectedOrder.orderNumber}</div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-xs text-gray-500">Status</div>
                  <div className="text-sm font-medium capitalize">{selectedOrder.status}</div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-xs text-gray-500">Total</div>
                  <div className="text-sm font-medium">₹{selectedOrder.totalAmount.toLocaleString('en-IN')}</div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-xs text-gray-500">Items</div>
                  <div className="text-sm font-medium">{selectedOrder.items.length}</div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  {selectedOrder.items.map((item) => (
                    <div key={item.id} className="flex items-center gap-4">
                      <img src={item.productImage} alt={item.productName} className="w-14 h-14 rounded object-cover" />
                      <div className="flex-1">
                        <div className="text-sm font-medium text-gray-900">{item.productName}</div>
                        <div className="text-xs text-gray-600">Qty: {item.quantity}</div>
                      </div>
                      <div className="text-sm font-semibold">₹{item.price.toLocaleString('en-IN')}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex justify-end">
                <button onClick={() => setShowDetails(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Close</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showTrack && selectedOrder && (
        <TrackOrderModal order={selectedOrder} onClose={() => setShowTrack(false)} />
      )}
    </div>
  );
}

const TrackOrderModal = ({ order, onClose }: { order: Order; onClose: () => void }) => {
    ...
    const loadOrderLogs = async () => {
      try {
        ...
        const sampleLogs = [
          ...
          { label: 'Order Confirmed', time: new Date(new Date(order.createdAt).getTime() + 24 * 60 * 60 * 1000).toISOString() },
          { label: 'Order Shipped', time: new Date(new Date(order.createdAt).getTime() + 2 * 24 * 60 * 60 * 1000).toISOString() },
          { label: 'Out for Delivery', time: new Date(new Date(order.createdAt).getTime() + 3 * 24 * 60 * 60 * 1000).toISOString() },
        ];
        setLogs(sampleLogs);
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h3 className="text-xl font-semibold text-gray-900">Track Order #{order.orderNumber}</h3>
            <p className="text-sm text-gray-600">Real-time journey of your package</p>
          </div>
        ...
        <div className="p-6 space-y-8">
          ...
          <div>
            <div className="relative h-3 bg-gray-200 rounded-full overflow-hidden">
              <div className="absolute left-0 top-0 h-3 bg-blue-500" style={{ 
              ...
              {checkpoints.map((_, index) => (
                <div 
                  ...
                  style={{ 
                    ...
                    backgroundColor: index <= currentCheckpointIndex ? '#3b82f6' : '#d1d5db'
                  }}
                ></div>
              ))}
            ...
            <div className="flex justify-between mt-2 text-xs text-gray-500">
              {checkpoints.map((checkpoint, index) => (
                <span 
                  key={index}
                  className={index <= currentCheckpointIndex ? 'text-blue-600 font-medium' : ''}
                >
                  {checkpoint.label}
                </span>
              ))}
            ...
            {logs.map((log, idx) => (
              <div key={idx} className="flex items-start gap-4">
                <div className="flex flex-col items-center">
                  <div className={`w-3 h-3 rounded-full ${idx <= currentCheckpointIndex ? 'bg-blue-600' : 'bg-gray-300'}`}></div>
                  {idx < logs.length - 1 && (
                    <div className={`w-[2px] flex-1 ${idx < currentCheckpointIndex ? 'bg-blue-200' : 'bg-gray-200'}`}></div>
                  )}
                </div>
                <div>
                  <div className="font-medium text-gray-900">{log.label}</div>
                  <div className="text-sm text-gray-600">{new Date(log.time).toLocaleString()}</div>
                </div>
              </div>
            ))}
          </div>
